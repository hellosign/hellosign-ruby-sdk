  ...
  class SubSigningOptions
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `HelloSign::SubSigningOptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.merged_attributes.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `HelloSign::SubSigningOptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # @param [Object] default_type Object to be assigned
    def default_type=(default_type)
      validator = EnumAttributeValidator.new('String', ["draw", "phone", "type", "upload"])
      unless validator.valid?(default_type)
        fail ArgumentError, "invalid value for \"default_type\", must be one of #{validator.allowable_values}."
      end
      @default_type = default_type
    end
